# Flask application for Employee Directory on GCP
import json
import os
import subprocess
import requests
...
def get_instance_document():
    try:
        ...
        r = requests.get("http://metadata.google.internal/computeMetadata/v1/instance/zone", headers=headers, timeout=1)
        r.raise_for_status()
        zone = r.text.split('/')[-1]
        
        r = requests.get("http://metadata.google.internal/computeMetadata/v1/instance/id", headers=headers, timeout=1)
        r.raise_for_status()
        instance_id = r.text
        
        return { "availabilityZone" : zone,  "instanceId" : instance_id }
    except:
        print(" * Instance metadata not available")
        return { "availabilityZone" : "us-fake-1a",  "instanceId" : "i-fakeabc" }


if "DATASTORE_MODE" in os.environ:
    import database_datastore as database
else:
    import database
...
def home():
    ...
    bucket = storage_client.bucket(config.PHOTOS_BUCKET)
        ...
        for employee in employees:
            try:
                if "object_key" in employee and employee["object_key"]:
                    blob = bucket.blob(employee["object_key"])
                    employee["signed_url"] = blob.generate_signed_url(
                        version="v4",
